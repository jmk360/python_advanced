...
# 매직메소드 실습
# 파이썬 중요한 핵심 프레임워크 -> 시퀀스(Sequence), 반복(Iterator), 함수(Functions), 클래스(class)
...
class Student:
    ...
    # 반드시 True, False를 리턴해야만 하는 것은 아니다. 함수 안에는 내 마음대로 하면 된다.
    def __ge__(self, x): 
    ...
    def __sub__(self, x):
        ...
        return self._height - x._height # abs(self._height - x._height)
...
# 벡터(Vector) : 벡터는 데이터 분석이나 머신러닝 쪽에서는 반드시 학습을 해야하는 개념이다.
class Vector(object):
