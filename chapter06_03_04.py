...
def main(separate_many):
    ...
    # 결과 건수
    # ProcessPoolExcutor : GIL 우회, 변경 후 -> os.cpu_count() -> 멀티프로세싱은 내부적으로 cpu core 개수를 가지고 와서 실행을 한다.
    # -> 조금 빠르게 전처리 할게 있으면, ProcessPoolExcutor(멀티 프로세싱)을 활용해서 gil을 피한다음에 사용하는 것이 훨씬 빠르게 실행이 된다.
    # ThreadPoolExcutor : GIL 종속
    # 멀티 프로세싱은 예를 들어 단기간의 딥러닝의 고속연산이나, 퓨리에 연산이나 등등 의 작업을 할때 쓰는 것이 좋다.
    ...
    # 최종 결과 출력
    # print(msg.format(list(list(futures_list)), end_tm))
